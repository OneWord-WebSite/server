...
const UC_PUBLIC_KEY = require('../../config/config')['UC_PUBLIC_KEY'];
const UC_PRIVATE_KEY = require('../../config/config')['UC_PRIVATE_KEY'];
const UC_PROXY_SUFFIC = require('../../config/config')['UC_PROXY_SUFFIC'];

var AuthClient = function(req) {
	this.req = req;
	this.public_key = UC_PUBLIC_KEY;
	this.private_key = UC_PRIVATE_KEY;
	this.proxy_suffix = UC_PROXY_SUFFIC;
	this.authorization = "";
}
...
AuthClient.prototype.PostUploadHit = function(callback) {
	...
	function get_etag(etag) {
		...
		var urlraw = 'http://' + self.req.bucket + self.proxy_suffix + self.req.url_path_params + '?Hash=' + etag + '&FileName=' + self.req.key + '&FileSize=' + self.req.file_size;;
		var urlstr = utf8.encode(urlraw);
